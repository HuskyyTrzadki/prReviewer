...
14. run overall design QA and copy review across the landing page and dashboard so spacing, hierarchy, tone, labels, and empty/error/loading text feel deliberate and consistent.
15. ensure performance budgets are enforced on the landing page, with minimal client JavaScript, optimized assets, and strong mobile Lighthouse results.
16. ensure test coverage is done for URL parsing, scoring payload/schema validation, critical UI states, and at least one end-to-end happy path.
17. ensure caching is done for repeated repository analyses so rate-limit pressure, latency, and LLM cost stay under control.
18. ensure code quality and wow-factor are done through cleanup, accessibility passes, motion polish, visual consistency, and one memorable landing detail added at the end.
