...
- **results** — repository summary, aggregate score cards, charts, same-tab result restoration, and the sortable/filterable PR list.
...
14. [x] run overall design QA and copy review across the landing page and dashboard so spacing, hierarchy, tone, labels, and empty/error/loading text feel deliberate and consistent.
    Milestone context: The first results-page QA pass is now complete. `src/features/results/results-dashboard-success.tsx` was reworked so the repository score reads as a stronger hero element, the summary panel gained a lightweight radar visualization through `src/features/results/results-score-radar-chart.tsx`, and the supporting breakdown copy/cards were tightened so the top section feels less like raw backend output. The PR preview and results list were also polished in-place: preview cards now use clearer metadata grouping, semantic diff stat colors, and stronger actions, while `src/features/results/*` gained a more deliberate filter tray, denser desktop score cells, upgraded mobile cards, and more product-quality helper/empty/sorting copy without changing the underlying filter or sort behavior. The latest pass also made sorting a first-class control in the filter tray through `src/features/results/results-table-toolbar.tsx`, moved the reset action next to the active filters instead of leaving it detached, and added a closing homepage CTA at the end of the dashboard so users can immediately start another repository analysis without dead-ending on the results view.
...
20. [x] ensure code quality.
    Milestone context: The first frontend code-quality pass is now complete around the live results experience. `src/features/results/results-table-desktop.tsx` was corrected to avoid invalid whitespace inside `<colgroup>`, which removed a real hydration error reported by the Next.js runtime on `/results/[repoId]`. The results restore flow in `src/features/results/results-dashboard-shell.tsx` no longer depends on a mount-only `useEffect`; session-backed state is now read through a cached snapshot path in `src/features/results/results-session.ts`, which removes an unnecessary extra render while keeping the same-tab restore model intact. The results state layer was also tightened: `src/features/results/results-table-state.ts` is now a small public barrel under the repo’s 200-line limit, with parsing/query helpers and filter/sort helpers split into focused modules, and select handling uses explicit narrowing helpers instead of broad casts. `ScoreCircle` and the shared score denominator were also moved out of feature-local code into `src/shared/ui/score-circle.tsx` and `src/shared/config/scoring.ts`, which makes `shared/` real for code already reused by both landing and results instead of leaving that boundary as empty placeholders. The former `results-dashboard` and `results-table` folders were then consolidated into one `src/features/results/*` feature root so the route, session state, summary UI, and sortable PR list now live under the same boundary they already shared in practice. `repo-input` now also follows a clearer split: `ui/` owns the form and loading panel, `model/` owns URL field/loading state logic, and `lib/` owns the analyze-route client plus the same-tab analysis starter flow, which removes API/session orchestration from the form component itself. Finally, `src/features/results/results-score-radar-chart.tsx` now narrows radar-axis labels through a real dimension guard instead of indexing by cast, so the client-side FE surface is cleaner and more type-defensive without changing behavior.
21. [x] add readme
    Milestone context: `README.md` now documents the product as `PR Reviewer AI` instead of the default Next.js starter template. The setup section reflects the actual repo scripts from `package.json` (`dev`, `build`, `lint`, `test`, `storybook`, `knip`), explains that `.env.local` must define `GOOGLE_API_KEY` and may optionally define `GITHUB_TOKEN`, and links future readers to both `BigPicturePlan.md` and `MyThinkingProcess.md` for product/build context. The runtime requirement is also enforced more explicitly now: `src/app/api/analyze/route.ts` calls the shared guard in `src/features/pr-analysis/lib/gemini-client.ts` before any GitHub work starts, so a missing Gemini key fails fast with the real configuration error instead of degrading into a generic analysis failure.
