...
20. [x] ensure code quality.
    Milestone context: The first frontend code-quality pass is now complete around the live results experience. `src/features/results-table/results-table-desktop.tsx` was corrected to avoid invalid whitespace inside `<colgroup>`, which removed a real hydration error reported by the Next.js runtime on `/results/[repoId]`. The results restore flow in `src/features/results-dashboard/results-dashboard-shell.tsx` no longer depends on a mount-only `useEffect`; session-backed state is now read through a cached snapshot path in `src/features/results-dashboard/results-session.ts`, which removes an unnecessary extra render while keeping the same-tab restore model intact. The results-table state layer was also tightened: `src/features/results-table/results-table-state.ts` is now a small public barrel under the repo’s 200-line limit, with parsing/query helpers and filter/sort helpers split into focused modules, and select handling uses explicit narrowing helpers instead of broad casts. `ScoreCircle` and the shared score denominator were also moved out of `results-dashboard` into `src/shared/ui/score-circle.tsx` and `src/shared/config/scoring.ts`, which makes `shared/` real for code already reused by both landing and results instead of leaving that boundary as empty placeholders. Finally, `src/features/results-dashboard/results-score-radar-chart.tsx` now narrows radar-axis labels through a real dimension guard instead of indexing by cast, so the client-side FE surface is cleaner and more type-defensive without changing behavior.
21. [x] add readme
    Milestone context: `README.md` now documents the product as `PR Reviewer AI` instead of the default Next.js starter template. The setup section reflects the actual repo scripts from `package.json` (`dev`, `lint`, `test`, `storybook`, `knip`), explains that `.env.local` must define `GOOGLE_API_KEY` and may optionally define `GITHUB_TOKEN`, and makes the runtime requirement explicit by pointing to the existing hard error in `src/features/pr-analysis/lib/gemini-client.ts` when Gemini is configured without an API key. This gives future agents and reviewers one source of truth for local setup, tooling, and the GitHub-vs-Gemini credential split.
