...
9. [x] Add the LLM scoring pipeline with structured output validation and repository-level aggregation.
   Milestone context: The analyze backend now carries a full scoring pipeline shape even before the real provider connection milestone. `prepare-repository-scoring-source.ts` trims the broader GitHub sample down to the first 8 PRs, attaches lightweight changed-file context for each one, and hands that evidence into `run-repository-scoring.ts`, which validates raw score payloads with Zod, keeps successful PR scores, and records skipped PRs with typed reasons instead of failing the whole analysis on one bad result. Score normalization and repository aggregation now live on the server in `score-repository-analysis.ts`, so `/api/analyze` can already return a stable results payload with repository summary metrics, per-PR scores, and skipped counts; for now the actual score values come from the temporary local scorer in `score-pull-request-stub.ts`, while the real LLM connection remains intentionally deferred to step 10.
10. Connect the server analysis flow to a real LLM provider with a single server-side scoring client, environment-based configuration, and structured response handling.
    Milestone context: The backend should now have a real LLM connection instead of only local scoring contracts and orchestration. A single server-only client in `src/features/pr-analysis` should call the chosen model using environment configuration, keep the runtime on Node.js, and return validated structured scoring data for the PR scoring pipeline. This milestone stays intentionally narrow: one provider, one fixed model, one request shape, and clear failure handling for timeout, provider errors, and invalid structured output. It should not add provider abstraction, streaming, multi-model fallback, prompt versioning infrastructure, or any client-side AI access.
11. Persist analysis results under a deterministic key and expose a stable results route for revisits and shareability.
12. Build the dashboard shell with total score, dimension breakdown, repository summary, and loading/error/empty states.
13. Implement the PR results list with sorting, filtering, score display, metadata, and external diff links.
14. ensure performance budgets are enforced on the landing page, with minimal client JavaScript, optimized assets, and strong mobile Lighthouse results.
15. ensure test coverage is done for URL parsing, scoring payload/schema validation, critical UI states, and at least one end-to-end happy path.
16. ensure caching is done for repeated repository analyses so rate-limit pressure, latency, and LLM cost stay under control.
17. ensure code quality and wow-factor are done through cleanup, accessibility passes, motion polish, visual consistency, and one memorable landing detail added at the end.
